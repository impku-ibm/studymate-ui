  ...
  return (
    <div className="h-screen flex overflow-hidden bg-slate-100">
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-56 transform transition-transform lg:relative lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <aside className="w-56 h-full bg-gradient-to-b from-indigo-900 to-indigo-800 text-slate-200 px-4 py-6 overflow-y-auto">
          <div className="mb-8">
          ...
          <nav className="space-y-1">
            <NavLink onClick={handleNav} to="/student" end className={({ isActive }) => `${linkClass} ${isActive ? "bg-indigo-600 text-white" : "hover:bg-indigo-700"}`}>Dashboard</NavLink>
            <NavLink onClick={handleNav} to="/student/attendance" className={({ isActive }) => `${linkClass} ${isActive ? "bg-indigo-600 text-white" : "hover:bg-indigo-700"}`}>Attendance</NavLink>
            <NavLink onClick={handleNav} to="/student/fees" className={({ isActive }) => `${linkClass} ${isActive ? "bg-indigo-600 text-white" : "hover:bg-indigo-700"}`}>Fees</NavLink>
            <NavLink onClick={handleNav} to="/student/results" className={({ isActive }) => `${linkClass} ${isActive ? "bg-indigo-600 text-white" : "hover:bg-indigo-700"}`}>Results</NavLink>
          </nav>
          <div className="mt-8">
